...
async def get_style_summary(redis_client, user_id: int) -> str | None:
    ...
    if profanity_rate > 0.3:
        traits.append(
            "часто использует лёгкий мат как манеру речи — можно отвечать так же неформально "
            "и с юмором, но НЕ грубить и НЕ выражать раздражение/враждебность к нему или к чату",
        )
...
async def generate_style_summary_llm(
    ...
    prompt = (
        ...
        "этому пользователю. ВАЖНО: инструкция может разрешать неформальный тон и лёгкий мат "
        "как манеру речи, но НИКОГДА не должна советовать боту грубить, раздражаться, "
        "оскорблять или выражать враждебность к пользователю или к чату. "
        "Если стиль нейтральный — ответь НЕТ."
    )
