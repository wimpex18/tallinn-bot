...
async def generate_proactive_message(
    ...
    """Generate a proactive message based on recent chat context.

    Called by the scheduled job (not by the observer handler).
    Returns None if the LLM decides there's nothing useful to add.
    """
    context_block = "\n".join(recent_messages[-15:])
    hour = datetime.now(TALLINN_TZ).hour

    # Give the model a sense of time for relevant suggestions
    if 8 <= hour < 12:
        time_hint = "Сейчас утро."
    elif 12 <= hour < 17:
        time_hint = "Сейчас день."
    elif 17 <= hour < 21:
        time_hint = "Сейчас вечер."
    else:
        time_hint = ""
    ...
    prompt = (
        ...
        f"Ты хочешь вернуться в разговор. Варианты:\n"
        f"- Дополнить обсуждаемую тему полезной информацией или мнением\n"
        f"- Предложить что-то актуальное (мероприятие, место, новость в Таллинне)\n"
        f"- Задать интересный вопрос по теме разговора\n\n"
        f"Напиши ОДНО короткое неформальное сообщение (1-3 предложения, без эмодзи). "
        f"Будь как обычный участник чата, не как бот и не как ассистент. "
        f"НЕ начинай с приветствий типа 'привет', 'кстати'. "
        f"Сообщение должно быть естественным продолжением разговора. "
        f"Если контекст слишком старый или скучный — ответь только НЕТ."
    )
    ...
    try:
        ...
        if not result or "НЕТ" in result.upper() or len(result) < 5:
            return None
